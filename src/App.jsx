...
function getFloatingPanelTitle(panelName = activePanel) {
  ...
  if (panelName === "build") {
    if (!selectedBuildCategory) return "建設メニュー";
    if (selectedBuildCategory === "住宅" && selectedHousingType) return `建設：${selectedHousingType}`;
...
function getLoanConsultationQualityRate(consultationPower) {
  const power = Math.max(0, Math.round(Number(consultationPower) || 0));
  return Math.max(0, Math.min(1, (power - 35) / 85));
}
...
function calculateLoanConsultationReport(consultation) {
  ...
  const qualityRate = getLoanConsultationQualityRate(consultationPower);
  const bankConservatism = bank.id === "nonbank" ? 0.88 : bank.id === "regional" ? 0.82 : bank.id === "shinkin" ? 0.76 : 0.68;
  const truePracticalLimit = Math.max(0, baseLimit * bankConservatism);

  /*
    v163 融資相談方針：
    ・相談力が低い社員ほど、実際に狙える上限よりかなり保守的な金額を回答する。
    ・相談力が高い社員ほど、銀行が見ている実務上限に近い金額を引き出す。
    ・低能力社員の相談結果では、コメントにも実際の上限額を直接出さない。
  */
  const guidanceRate = Math.max(0.28, Math.min(1.0, 0.32 + qualityRate * 0.68));
  const recommendedAmount = Math.max(0, Math.round(truePracticalLimit * guidanceRate / 100) * 100);

  const lowerRate = Math.max(0.45, Math.min(0.9, 0.5 + qualityRate * 0.4));
  const upperRate = Math.max(1.05, Math.min(1.75, 1.65 - qualityRate * 0.55));
  const estimatedMin = Math.max(0, Math.round(recommendedAmount * lowerRate / 100) * 100);
  const estimatedMax = Math.max(estimatedMin, Math.round(recommendedAmount * upperRate / 100) * 100);
  ...
  if (baseLimit <= 0) riskNotes.push("追加借入余力がほぼない");
  if (qualityRate < 0.35) riskNotes.push("担当の相談精度が低く、かなり保守的な見立て");
...
async function startLoanConsultation(bankId) {
  ...
  const ok = window.confirm(
    `${bank.name}へ融資相談を行いますか？\n\n` +
      ...
      `期間: 1ヶ月\n\n` +
      `※相談結果は担当社員の相談力により精度が変わります。低い相談力ではかなり保守的な見立てになります。`
  );
...
async function borrowFromBank(bankId, overrideAmount = null, consultationReport = null) {
  ...
  if (amount > (capacity?.remainingLimit ?? 0)) warningMessages.push("希望額が現在の審査水準を超える可能性があります");
...
function calculateLoanReviewResult(application, currentLoans = loans) {
  ...
  const consultationExecutionGap = hasConsultationSupport
    ? Math.max(0, consultationPower - negotiationPower)
    : 0;
  const consultationExecutionPenalty = hasConsultationSupport
    ? Math.min(26, consultationExecutionGap * 0.45)
    : 0;
  ...
  if (!canApprove) {
    ...
    if (consultationExecutionPenalty >= 8) reasons.push(`相談担当より申請担当の交渉力不足`);
  ...
  <div className={`recruit-avatar recruit-avatar-${selectedApplicant.gender === "female" ? "female" : "male"}`}>
    <span>{selectedApplicant.name.slice(0, 1)}</span>
              ...
              <div className="recruit-profile-main">
                ...
                <div className="recruit-special-box recruit-special-box-expanded">
                  <span className="recruit-special-label">特性</span>
                  {selectedApplicantSkills.length > 0 ? (
                    <div className="recruit-special-list">
                      {selectedApplicantSkills.map((skill) => (
                        <div className="recruit-special-item" key={skill.id}>
                    ...
                    <strong className="recruit-special-none">なし</strong>
                  )}
                </div>
              </div>
            </div>
        ...
        <tbody>
          {(() => {
            ...
            Object.values(RIVAL_COMPANIES).forEach((company) => {
              ...
              const rivalPayroll = (hqTile?.rivalEmployees ?? []).reduce((sum, employee) => sum + getEmployeeSalary(employee), 0);
